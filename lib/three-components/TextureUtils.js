...
export default class TextureUtils extends EventDispatcher {
    ...
    generateDefaultEnvironmentMap(config = {}) {
        ...
        let environmentMap;
        if (config.pmrem) {
            environmentMap = this.pmremPass(cubemap, this.config.defaultEnvironmentPmremSamples, this.config.defaultEnvironmentPmremSize);
            cubemap.dispose();
        }
        else {
    ...
    async generateEnvironmentTextures(url, config = {}) {
        ...
        try {
            ...
            if (config.pmrem) {
                environmentMap = this.pmremPass(skybox.texture, this.config.pmremSamples, this.config.pmremSize);
            }
            else {
