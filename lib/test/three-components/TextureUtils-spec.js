...
suite('TextureUtils', () => {
    ...
    suite('generateEnvironmentTextures', () => {
        ...
        test('returns an environmentMap and skybox texture from an HDR url', async () => {
            ...
            expect(textures.skybox.texture.isTexture).to.be.ok;
            expect(textures.environmentMap.isTexture).to.be.ok;
            expect(textureMatchesMeta(textures.skybox.texture, { mapping: 'Cube', url: HDR_EQUI_URL }))
                .to.be.ok;
            expect(textureMatchesMeta(textures.environmentMap, { mapping: 'Cube', url: HDR_EQUI_URL }))
                .to.be.ok;
        });
        test('returns an environmentMap and skybox texture from url with PMREM', async () => {
            ...
            expect(textureMatchesMeta(textures.skybox.texture, { mapping: 'Cube', url: EQUI_URL }))
                .to.be.ok;
            expect(textureMatchesMeta(textures.environmentMap, { mapping: 'PMREM', url: EQUI_URL }))
                .to.be.ok;
        });
        test('returns an environmentMap and skybox texture from an HDR url', async () => {
            textures = await textureUtils.generateEnvironmentTextures(HDR_EQUI_URL, {
            ...
            expect(textureMatchesMeta(textures.skybox.texture, { mapping: 'Cube', url: HDR_EQUI_URL }))
                .to.be.ok;
            expect(textureMatchesMeta(textures.environmentMap, { mapping: 'PMREM', url: HDR_EQUI_URL }))
                .to.be.ok;
        });
